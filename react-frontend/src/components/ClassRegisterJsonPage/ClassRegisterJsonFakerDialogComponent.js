...
import classRegisterJsonFakerFactory from "./classRegisterJsonFakerFactory";
...
const ClassRegisterJsonFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = classRegisterJsonFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("classRegisterJson").create(elem));
...
export default connect(null, mapDispatch)(ClassRegisterJsonFakerDialogComponent);
