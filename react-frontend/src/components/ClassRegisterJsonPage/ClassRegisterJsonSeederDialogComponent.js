...
import seederData from "./classRegisterJson.seeder.json";
...
const ClassRegisterJsonSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("classRegisterJson").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ClassRegisterJsonSeederDialogComponent);
